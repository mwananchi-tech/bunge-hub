...
export function ModelBadge({ model }: Props) {
  ...
  return (
    <span className="relative group inline-flex items-center gap-1 text-xs mt-2 cursor-default"
      ...
      {/* Tooltip */}
      <span className="absolute top-full left-0 mt-2 w-56 rounded-lg px-3 py-2 text-xs text-left
        ...
        This summary was generated using an open-source AI language model ({model}) and may not be fully accurate.
        Always refer to the original transcript for the authoritative record.
      </span>
    </span>
  );
}
