...
import { listSittings, countSittings } from "~/lib/queries/sittings.server";
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  return { sittings: rows.slice(0, LIMIT), house, year, page, hasMore, totalPages,
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12">
      <div className="mb-8">
        ...
        <p className="text-sm" style={{ color: "var(--color-muted)" }}>13th Parliament</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-8">
        ...
        <div className="flex gap-1">
          <Link to={yearUrl(undefined)}
                className="px-3 py-1.5 text-sm rounded"
                style={{ backgroundColor: !year ? "var(--color-surface)" : "transparent",
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs px-2 py-0.5 rounded font-medium"
                      style={{ backgroundColor: s.house === "Senate" ? "var(--color-accent)18" : "var(--color-gold)18",
              ...
              {s.summary && (
                <p className="text-xs leading-relaxed line-clamp-2 max-w-2xl mb-1"
